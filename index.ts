...
import {formatDateHuman, formatCronHuman, formatTaskDueTime, getCurrentTime} from './dateUtils';
...
Routines/Schedule:
${activeRoutines.map(r => `id: ${r.id} cron: ${r.cron} (${formatCronHuman(r.cron)}) defaultAnnoyance: ${r.defaultAnnoyance} name: ${r.name}`).join('\n') || 'no active routines'}
